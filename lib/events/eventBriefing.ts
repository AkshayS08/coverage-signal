 ...
 * headline trigger's own evidence (date-prefixed, so it's still anchored
 * even in degraded mode) so it degrades gracefully if the Sonnet call
 * fails or times out. Unlike the Sonnet version this doesn't synthesize
 * (no model to do the synthesis), so it can run long — it's a
...
export function templateEventBriefing(card: FlashCard): DraftedEventBriefing {
  ...
  return {
    what: `${datePrefix}${t.evidence ?? `${t.triggerName} flagged (${t.needType}).`}`,
