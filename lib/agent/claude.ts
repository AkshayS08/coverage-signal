...
- scheduleSequence / priorScheduleSequence / balanceSheetDebtCaptions — ONLY for the "debt-maturity" trigger. Leave all three at their empty default ([], [], []) for every other trigger.
  ...
  - scheduleSequence is an ORDERED array, IN PRINTED ORDER — literally the order these lines appear on the page, top to bottom, exactly as printed. Each entry has a kind:
    - "row": one debt instrument/tranche/category line (e.g. "Term Loan A-2", "Commercial paper", "Senior unsecured notes payable through 2095"). label = the instrument's own name.
      TYPOGRAPHY IS NOT THE TEST. A row is one instrument's own statement of what is owed on it, and a filing may print that as a table row, as a BULLETED ITEM, or as a SENTENCE. All three are rows here and all three are transcribed the same way, in printed order. When the note presents its instruments as bullets — "• $ 700 million of aggregate principal amount of 1.65 % senior secured notes due in September, 2026" — each bullet is one row: label the instrument as the note names it, amount the principal that bullet states, rate and maturityDate from that same bullet and no other. A note that lists five instruments as five bullets under the sentence "we had combined aggregate principal of $ 3.0 billion from the following senior secured notes:" is a schedule with five rows and a subtotal, printed without a table, and it is transcribed as exactly that. Do not skip a disclosure because it is not tabulated; do not merge bullets; do not summarise them. Also carries rate, seniority (verbatim from the note's own section header, null if not stated), maturityDate + dateGranularity (same copy-never-compute rules as eventDate above — bare year stays bare, and BOTH ARE NULLABLE: null is the correct answer whenever this specific row states no maturity — a catch-all line like "Other debt (effective interest rate of 4.9%)" often states a rate with no maturity year anywhere next to it; do not default to the reporting period or any other plausible-looking year — that is fabrication, independently checked downstream against this row's own sourceLine).
